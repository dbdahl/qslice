# creating the trials for the gamma target
# author: Sam Johnson
...
rate <- c(0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1, 2)#c(0.5, 1, 1.5, 2, 2.5, 3)
...
miscPseudoTargets <- list(
  pseudo_t_list(loc = 2.5, sc = 1, degf = 1, lb = 0, name = 'Man'),
  pseudo_t_list(loc = 0.05328, sc = 1.2702, degf = 1, lb = 0, name = 'Man'),
  pseudo_t_list(loc = 1.73, sc = 1.69, degf = 5, lb = 0, name = 'Man'),
  pseudo_t_list(loc = 1.59, sc = 1.84, degf = 5, lb = 0, name = 'Man'),
  pseudo_t_list(loc = 1.56, sc = 1.89, degf = 5, lb = 0, name = 'Man'),
  pseudo_t_list(loc = 2.5, sc = sqrt(2.5), degf = 1, lb = 0, name = 'MM')
)

# getting burn in draws to fit pseudo target
burnin_metrics <- rgamma(5e3, 2.5 ,1)
...
optimSamplesT <- sapply(1:nrow(testCoeffsDf), FUN = \(i) {
  ...
  psuedoFit <- opt_t(samples = burnin_metrics, nbins = 30, coeffs = coeffs, lb = 0, type = 'samples')
...
optimSamplesTAUC <- sapply(1:nrow(testCoeffsDf), FUN = \(i) {
  coeffs <- testCoeffsDf[i,]
  coeffs <- c(coeffs[1,1], coeffs[1,2])
  psuedoFit <- opt_t(samples = burnin_metrics, nbins = 30, coeffs = coeffs, lb = 0, type = 'samples', use_meanSliceWidth = FALSE)
  temp <- pseudo_t_list(loc = psuedoFit$pseu$loc[[1]],sc = psuedoFit$pseu$sc[[1]], degf = psuedoFit$pseu$degf[[1]], lb = 0,
...
optimT <- sapply(1:nrow(testCoeffsDf), FUN = \(i) {
  coeffs <- testCoeffsDf[i,]
  coeffs <- c(coeffs[1,1], coeffs[1,2])
  psuedoFit <- opt_t(target = truth, nbins = 30, coeffs = coeffs, lb = 0, type = 'function')
  temp <- pseudo_t_list(loc = psuedoFit$pseu$loc[[1]],sc = psuedoFit$pseu$sc[[1]], degf = psuedoFit$pseu$degf[[1]], lb = 0,
...
optimTAUC <- sapply(1:nrow(testCoeffsDf), FUN = \(i) {
  coeffs <- testCoeffsDf[i,]
  coeffs <- c(coeffs[1,1], coeffs[1,2])
  psuedoFit <- opt_t(target = truth, nbins = 30, coeffs = coeffs, lb = 0, type = 'function', use_meanSliceWidth = FALSE)
  temp <- pseudo_t_list(loc = psuedoFit$pseu$loc[[1]],sc = psuedoFit$pseu$sc[[1]], degf = psuedoFit$pseu$degf[[1]], lb = 0,
