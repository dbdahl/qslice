# A function to combine data from all the methods
# Author: Sam Johnson

combine_func <- function(stepping_out_metrics, latent_metrics, gess_metrics, rand_walk_metrics, transform_metrics, metric) {
  # finding the best w
  ...
  tempSteppingOut <- stepping_out_metrics %>% 
    dplyr::filter(w == bestW) %>% select(eval(metric),w,userTime,sysTime,elapsedTime) %>% 
    dplyr::rename('par' = 'w') %>% 
  ...
  tempLatent <- latent_metrics %>% 
    dplyr::filter(rate == bestRate) %>% select(eval(metric), rate, userTime, sysTime, elapsedTime) %>% 
  ...
  if( metric == 'sampPsec' ) {
    bestGESS <- temp$par[which.max(temp[,metric])]
  } else {
    bestGESS <- temp$par[which.min(temp[,metric])]
  }
  ...
  tempGess <- gess_metrics %>% 
    ...
    dplyr::select(eval(metric), par, userTime, sysTime, elapsedTime)
  ...
  tempRandWalk <- rand_walk_metrics %>% 
    ...
    select(eval(metric), c, userTime, sysTime, elapsedTime) %>% 
    mutate(c = paste0('c=',c)) %>% 
    dplyr::rename('par' = 'c')
  tempRandWalk$method <- 'RandWalk'
  # creating a transform data frame
  tempTransform <- transform_metrics %>% 
    select(eval(metric), t, userTime, sysTime, elapsedTime) %>% 
    mutate(t = unlist(t)) %>% 
    mutate(
