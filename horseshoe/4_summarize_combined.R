...
dat$algoF <- factor(dat$algo, levels = rev(c("rw", "stepping", "latent",
                                             "imh AUC_samples", "imh MSW_samples",
                                             "gess AUC_samples", "gess MSW_samples",
                                             "Qslice AUC_samples", "Qslice MSW_samples")),
                    labels = rev(c("Random walk", "Steping out & shrinkage", "Latent slice",
                                   "Independence M-H: AUC", "Independence M-H: MSW",
                                   "Generalized elliptical slice: AUC", "Generalized elliptical slice: MSW",
                                   "Quantile slice: AUC", "Quantile slice: MSW"))
)
...
## tuning parameters
ggplot(dat %>% filter(grepl("tau2_marg$", target), algo == "rw"),
...
ggplot(dat %>% filter(grepl("tau2_marg-log", target), algo == "rw"),
       aes(x = tuneParam, y = algoF, fill = typeF)) +
  geom_violin() +  theme_bw() + theme(legend.position = "none") +
  xlab("Tuning parameter value") + ylab("") + labs(color = "", fill = "")
...
dat_plt_n <- dat %>% filter(n %in% c(40, 442)) %>%
...
plt <- ggplot(dat_plt_n %>% filter(grepl("tau2_marg", target)),
              aes(x = sampPsec_rel,
                  y = algoF, fill = typeF, color = target_tx, alpha = target_tx_alpha)) +
  geom_vline(xintercept = 1.0, col = "gray70") +
  geom_violin(draw_quantiles = 0.5, scale = "width") +
  theme_bw() + theme(legend.position = "none") +
  scale_alpha(guide = "none") +
  xlab("Effective samples per second") + ylab("") + labs(color = "", fill = "") +
...
plt_n <- plt + facet_grid(cols = vars(n_lab), scales = "fixed") + expand_limits(x = 0) +
  coord_cartesian(clip = "off") + theme(panel.spacing = unit(3.5, "lines")) +
  theme(plot.margin = unit(c(5.5, 50, 5.5, 0), "pt")) +
  theme(panel.border = element_blank()) + geom_blank(data = x_pad, aes(x = sampPsec_rel)) +
