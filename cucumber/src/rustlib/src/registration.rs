...
#[roxido]
fn slice_sampler(x: Rval, target: Rval, w: Rval, max: Rval, log: Rval) -> Rval {
    Rval::nil()
}
*/
...
#[no_mangle]
extern "C" fn R_init_cucumber_librust(info: *mut rbindings::DllInfo) {
    let mut call_routines = Vec::with_capacity(1);
    let mut _names: Vec<std::ffi::CString> = Vec::with_capacity(1);
    _names.push(std::ffi::CString::new(".slice_sampler").unwrap());
    call_routines.push(rbindings::R_CallMethodDef {
        name: _names.last().unwrap().as_ptr(),
        fun: unsafe { std::mem::transmute(crate::slice_sampler as *const u8) },
        numArgs: 5,
    });
    call_routines.push(rbindings::R_CallMethodDef {
        name: std::ptr::null(),
