...
py <- dgamma(grid, shape = 2.5, rate = 1)
...
log_pdf <- c('dgamma(x, shape = 2.5, rate = 1, log = TRUE)',
             'dnorm(x, mean = 0, sd = 4, log = TRUE)')

inv_cdf <- c('qgamma(u, shape = 2.5, rate = 1)',
             'qnorm(u, mean = 0, sd = 4)')

find_grid <- c('seq(from = 0, to = qgamma(0.99999, shape = 2.5, rate = 1), length.out = 1000)',
               'seq(from = qnorm((1-.99999)/2, mean = 0, sd = 4),
               to = qnorm((1-.99999)/2, mean = 0, sd = 4, lower.tail = FALSE), length.out = 1000)')
...
# saving the power test
pdf(file = "../images_slice_sampler_comp/curve3_transform_power.pdf")
extract_pvals(list_hldr = list_hldr, dist_df = dist_df)
dev.off()
...
pdf(file = "../images_slice_sampler_comp/curve3_transform.pdf")
curve(
  fexp(x),
  col = 'red',
  xlim = c(xlim_range[1], xlim_range[2]),
...
transform_min_max <- results(transform_metrics, method = "Transform")
...
xtable::print.xtable(tab, file = '../images_slice_sampler_comp/curve3_transform_evaluation.tex')
...
xtable::print.xtable(tab, file = '../images_slice_sampler_comp/curve3_min_max.tex')
