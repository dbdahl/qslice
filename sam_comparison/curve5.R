...
grid <- seq(from = qt((1-.99999)/2, df = 3),
            to = qt((1-.99999)/2, df = 3, lower.tail = FALSE),
            length.out = 1000)
...
log_pdf <- c('dt(x, df = 5, log = TRUE)',
             'dnorm(x, mean = 0, sd = 3, log = TRUE)')

inv_cdf <- c('qt(u, df = 5)',
             'qnorm(u, mean = 0, sd = 3)')

find_grid <- c('seq(from = qt((1-.99999)/2, df = 5),
               to = qt((1-.99999)/2, df = 5, lower.tail = FALSE), length.out = 1000)',
...
# saving the power test
pdf(file = "../images_slice_sampler_comp/curve5_transform_power.pdf")
extract_pvals(list_hldr = list_hldr, dist_df = dist_df)
dev.off()
...
# evalTbl_stepping_out <- cbind(start_points_stepping_out, w_values_stepping_out) %>% round(.,1)
pdf(file = "../images_slice_sampler_comp/curve5_transform.pdf")
curve(
  fexp(x),
  col = 'red',
  xlim = c(xlim_range[1], xlim_range[2]),
...
xtable::print.xtable(tab, file = '../images_slice_sampler_comp/curve5_transform_evaluation.tex')
...
# saving the power test
pdf(file = "../images_slice_sampler_comp/curve5_rand_walk_power.pdf")
extract_pvals(list_hldr = list_hldr, dist_df = dist_df)
dev.off()
...
# evalTbl_stepping_out <- cbind(start_points_stepping_out, w_values_stepping_out) %>% round(.,1)
pdf(file = "../images_slice_sampler_comp/curve5_rand_walk.pdf")
curve(
  fexp(x),
  col = 'red',
  xlim = c(xlim_range[1], xlim_range[2]),
...
xtable::print.xtable(tab, file = '../images_slice_sampler_comp/curve5_min_max.tex')
