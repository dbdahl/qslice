...
library(doParallel) # to run the for loop in parallel
library(tidyverse)
library(magrittr)
library(stringr)
library(patchwork)
...
file_path <- 'practicePlots/' #'../images_slice_sampler_comp/sampler_eda/'
save.images <- TRUE 
heightVal <-  1.445
widthVal <- 2.83
...
for (i in 1:(length(curves) - 1)) {
  curve(fexp(f = curves[[i]], x = x), 
        xlim = range[[i]]$xrange,
        ylim = range[[i]]$yrange,
        ylab = "", xlab = '', main = curveNames[i], cex.main = 1.1)
}
...
output <- foreach(i = 1:7) %dopar% {
  ...
  curve(fexp(f = curves[[i]], x = x), xlim = range[[i]]$xrange, ylim = range[[i]]$yrange, main = curveNames[i], xlab = '', ylab = '', cex.main = 2.0)
  if(save.images) dev.off()
 
  ksFilterInd <- TRUE

  # this extracts all the draws
  draws_list <- lapply(files[stringr::str_detect(files, paste0('.*',i,'.*_metrics'))], extract_draws)
